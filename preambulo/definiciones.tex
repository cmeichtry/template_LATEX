% definiciones.tex
% Copyright (C) 2023 Cristian Meichtry
% https://github.com/cmeichtry/template_LATEX
%En este archivo se completan los datos que van a aparecer en la portada y en el informe, por ejemplo, nombres de los alumnos, fecha, logo de la universidad, etc. Solamente hay que completar los campos con la información necesaria y no modificar nada de las carátulas salvo que sea necesario (como la información relevante en caratula4.tex)

%--------------------Datos para automatizar--------------------%
\def \universidad{FACULTAD}
\def \unidad{NUMERO DE TP}
\def \carrera{CARRERA O DEPARTAMENTO}
\def \materia{MATERIA}
\def \titulo{TITULO}
\def \logofac{imagenes/logo.png} %cambiar el nombre del archivo segun la imagen que corresponda

\def \grupo{NUMERO DE GRUPO}

%%%%%%% nombres de los integrantes %%%%%%%
%%%%%%% integrante 1 %%%%%%%
\def \autorUNO{
    Integrante 1
}
\def \legajoUNO{
    Legajo 1
}
\def \mailUNO{
    mail2@bla.com
}

%%%%%%% integrante 2 %%%%%%%
\def \autorDOS{
    Integrante 2
}
\def \legajoDOS{
    Legajo 2
}
\def \mailDOS{
    mail2@bla.com
}

%%%%%%% integrante 3 %%%%%%%
\def \autorTRES{
    Integrante 3
}
\def \legajoTRES{
    Legajo 3
}
\def \mailTRES{
    mail3@bla.com
}

%%%%%%% integrante 4 %%%%%%%
\def \autorCUATRO{
    Integrante 4
}
\def \legajoCUATRO{
    Legajo 4
}
\def \mailCUATRO{
    mail4@bla.com
}

%%%%%%% integrante 5 %%%%%%%
\def \autorCINCO{
    Integrante 5
}
\def \legajoCINCO{
    Legajo 5
}
\def \mailCINCO{
    mail5@bla.com
}

%%%%%%% integrante 6 %%%%%%%
\def \autorSEIS{
    Integrante 6
}
\def \legajoSEIS{
    Legajo 6
}
\def \mailSEIS{
    mail6@bla.com
}

%\def \autores{
%\noindent
%    \textbf{GRUPO N° \grupo}\\
%    Integrante 1\\
%    Integrante 2\\
%    Integrante 3\\
%    Integrante 4\\
%    Integrante 5\\
%} %nombres de los alumnos (Se agrega numero de grupo de forma automática)


%%%%%%% nombres de los profesores %%%%%%%
\def \profesorUNO{
    Ing. Profesor 1
}

\def \profesorDOS{
    Ing. Profesor 2
}

\def \profesorTRES{
    Ing. Profesor 3torUNO
}
%\def \profesores{
%    Ing. Profesor 1\\
%    Ing. Profesor 2\\
%}

%%%%%%% nombres de los ayudantes %%%%%%%
\def \ayudanteUNO{
    Ayudante 1
}

\def \ayudanteDOS{
    Ayudante 2
}

\def \ayudanteTRES{
    Ayudante 3
}

%\def \Colaboradores{ 
%    
%}

\def \fecha{
    Fecha de entrega  %se puede poner \today, pone la fecha de hoy (CHEQUEAR porque pone un dia o el otro según la hora de compilación)
}

%--------------------Funciones para formato y numeración--------------------%
\def \seteocontadores{
\setcounter{equation}{0} %empezar numeración desde 1 para ecuaciones
\setcounter{figure}{0} %empezar numeración desde 1 para figuras
\setcounter{table}{0} %empezar numeración desde 1 para tablas
}

\def \formatoapendice{
\renewcommand{\thetable}{\nombreapendice{}.\arabic{table}}
\renewcommand{\thefigure}{\nombreapendice{}.\arabic{figure}}
\rfoot{\nombreapendice{}.\thepage}
}

%--------------------Funciones para usar en mathmode--------------------%
\newcommand{\derpar}[2]{\frac{\partial #1}{\partial #2}} %derivada parcial
\newcommand{\der}[2]{\frac{d #1}{d #2}} %derivada "normal"
\newcommand{\paren}[1]{\left( #1 \right)} %Parentesis
\newcommand{\corch}[1]{\left[ #1 \right]} %corchetes
\newcommand{\llave}[1]{\left \{ #1 \right \}} %llaves
\newcommand{\abs}[1]{\lvert#1\rvert} %módulo
\newcommand{\supind}[2]{#1^{#2}} %supraindice
\newcommand{\subind}[2]{#1_{#2}} %supraindice
\newcommand{\ohm}{\Omega} %Escribe el simbolo Ω
\newcommand{\degree}{^\circ}  %Escribe el simbolo °
\newcommand{\grados}{^\circ}  %Escribe el simbolo °
\newcommand{\entonces}{\Rightarrow} %Escribe el simbolo ->
\newcommand{\sii}{\Leftrightarrow} %Escribe el simbolo <->

%--------------------Funciones para formato de texto--------------------%
\newcommand{\cursiva}[1]{\textit{#1}}
\newcommand{\negrita}[1]{\textbf{#1}}
\newcommand{\subrayar}[1]{\underline{#1}}
\newcommand{\comilladoble}[1]{``#1''} %Agrega comillas dobles
\newcommand{\comillasimple}[1]{`#1'} %Agrega comillas simples
